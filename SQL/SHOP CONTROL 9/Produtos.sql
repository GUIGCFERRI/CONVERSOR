SELECT 
PROD_SERV.ORDEM AS caracteristicas,
...
case when Prod_Serv.Codigo_barras = '' or Prod_Serv.Codigo_barras is null then Prod_Serv.Codigo_Adicional1 else Prod_Serv.Codigo_barras END AS barras,
...
left (Classe_Imposto.Nome,3) as elo,
...
LEFT JOIN S9_Real.dbo.Prod_Serv_Precos PSVVenda on Prod_Serv.Ordem=PSVVenda.Ordem_Prod_Serv AND PSVVenda.Ordem_Tabela_Preco=9 ---AQUI AJUSTA O CÓDIGO (ORDEM) DA TABELA DE PREÇO QUE DEVE USAR
...
left join S9_Real.dbo.Prod_Serv_precos PSVAtacado on Prod_Serv.ordem=PSVAtacado.Ordem_Prod_Serv AND PSVAtacado.Ordem_Tabela_Preco=8
...
left join Classe_Imposto on Classe_Imposto.Ordem = Prod_Serv.Ordem_Classe_Imposto_Saida 
LEFT JOIN S9_Real.dbo.Estoque_Atual ON Prod_Serv.Ordem=Estoque_Atual.Ordem_Prod_Serv and Estoque_Atual.Ordem_Filial = '1'
where Prod_Serv.ordem>0 
